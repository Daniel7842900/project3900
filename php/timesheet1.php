...
error_reporting(E_ALL);
  ...
  //echo $userId;
	$sql = "SELECT Category.Category_Code, Task.Task_Code,
          Weekly_Timesheet.Actuals, Weekly_Timesheet.ETC,
          Task.Task_ID
          FROM User
          INNER JOIN Project
...
<body>
<form action= "timesheet1.php" method ="post">
  <div class="dropdown">
...
?>
  ...
  <?php
    ...
    $taskID = $wt['Task_ID'];   
    //echo $taskID;
    if (isset($_SESSION['Actuals']) && isset($_SESSION['ETC'])){
  
      $charged = $_SESSION['Actuals'];
      $ETC = $_SESSION['ETC'];
...
/*      for ($c = 0, $count = count($_POST['Actuals']); $c < $count; $c++){
      ...
      for ($c = $j, $count = count($_SESSION['Actuals']); $c < $count; $c++){
        mysqli_query($con, "UPDATE Weekly_Timesheet SET Actuals = '$charged[$c]', ETC = '$ETC[$c]' WHERE Task_Task_ID = '$taskID'");
        ++$j;
        break;
      }
      ...
      /*if ($changeTimesheet){
        //echo "changed";
        echo "<meta http-equiv=\"refresh\" content = \"0; URL = timesheet1.php\">";
      }
      else{
        echo "not edited";
      }*/
